...
const JOB_SEEKER_TURNS = [
  ...
  {
    id: "js-eligibility",
    match: ["29", "m.a", "ma and", "b.ed", "bed", "tgt", "three years", "3 years", "patna", "delhi"],
    response: {
      answer:
  ...
  {
    id: "js-confirm",
    match: ["vice principal", "yes, the vice", "go ahead", "that one"],
    response: {
      answer:
  ...
  {
    id: "js-experience",
    match: ["science", "class 6", "classes 6", "science club", "exam scheduling", "coordinator", "government school"],
    response: {
      answer:
...
// Matches sequentially first — the expected next turn wins — then falls back to
// any unplayed turn so the conversation survives being asked out of order.
// Returns null when nothing matches, which sends the question to the live
// pipeline instead.
export function matchSessionTurn(script, text, playedIds = []) {
  if (!script) return null;
  const lower = String(text || "").toLowerCase();
  if (!lower.trim()) return null;
  const hits = (turn) => turn.match.some((needle) => lower.includes(needle.toLowerCase()));
  const remaining = script.turns.filter((turn) => !playedIds.includes(turn.id));
  if (remaining[0] && hits(remaining[0])) return remaining[0];
  return remaining.find(hits) || null;
}
