const STORAGE_KEY = "sahayak_preferences_v3";
...
export function setPreferences(patch) {
  ...
  window.dispatchEvent(new CustomEvent("sahayak:preferences", { detail: next }));
  return next;
}
...
export function preferenceControlsMarkup({ compact = false } = {}) {
  ...
  return `<div class="preference-controls${compact ? " compact" : ""}" aria-label="Sahayak preferences">
...
export function mountPreferenceControls(target, options = {}) {
  ...
  window.addEventListener("sahayak:preferences", onChange);
  return () => window.removeEventListener("sahayak:preferences", onChange);
}
